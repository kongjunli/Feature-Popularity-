...
class GetTitle():
    ...
    def modifyTitle(self,count,i):
        ...
        cursor=db.cursor()
        Content=[]
        sql='update spiderdb.paper set sumtopspecial=%f where id=%d'%(count,i)
        cursor.execute(sql) 
        db.commit()  
        db.close()
data = xlrd.open_workbook('C:\Users\kjl\Desktop\TOPWORDSsummary200000cixing.xls')
table=data.sheet_by_name(u'other special name')
topwords={}
for i in range(20):
